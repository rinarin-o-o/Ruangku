...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_jadwal_ruang'])) {
        $id_jadwal_ruang = mysqli_real_escape_string($conn, $_POST['id_jadwal_ruang']);
    } else {
        $_SESSION['error'] = 'ID Jadwal ruang tidak ditemukan.';
        header('Location: ../../jd_frm_edit_ruang.php?id_jadwal_ruang=' . $id_jadwal_ruang);
        exit;
    }
    ...
    if ($tgl_mulai && $waktu_mulai && $tgl_sekarang && $waktu_sekarang && $penanggungjawab && $pengguna && $acara) {
        // Update query
        $sql_update = "UPDATE jadwal_ruang 
        ...
        if (mysqli_query($conn, $sql_update)) {
            ...
            // Redirect to jadwal_ruang.php after update
